...
mod tests {
    ...
    fn parse_geometry_shapes() {
        ...
        match &geom[0].shape {
            ShapeDef::Cuboid { half_x, half_y, half_z } => {
        ...
        match &geom[1].shape {
            ShapeDef::Cylinder { half_height, radius } => {
    ...
    fn parse_shipped_dungeons_ron() {
        ...
        assert!(!file.templates.is_empty(), "shipped file should have at least one template");
    ...
    fn resolve_linked_entities_dangling_ref() {
        ...
        let defs = vec![
            make_def(1, InteractKindDef::Switch, Some(99)),
        ];
        let mut map = std::collections::HashMap::new();
        map.insert(1, 500);

        let resolved = resolve_linked_entities(&defs, &map);
        assert_eq!(resolved[0].linked_entity, None, "dangling linked_to should resolve to None");
    }
