    ...
    pub(super) fn execute_ability_action(
        ...
        match action {
            ...
            AbilityAction::ApplyDamageFrame => {
                ...
                let multi_data = self.state.combat.executions
                    .get(execution_id)
                    .and_then(|ctx| {
                        if let game_core::combat::skill::ResolvedTargeting::MultiLockOn { targets } = &ctx.targeting {
                ...
                if let Some((targets, caster_pos)) = multi_data {
                    ...
                    for (target, target_idx) in valid {
                        self.apply_hit_damage(entity, target, target_idx, ability_id, false, Some(execution_id));
                    }
                    for &target in &invalid {
                        self.emit_event(entity, EventPayload::LockOnCanceled { source: entity, target });
                    }
                    self.emit_event(entity, EventPayload::LockOnFired { source: entity, targets });
                ...
                if let Some((shape, offset)) = stored {
                    ...
                    let is_ground_target = !is_projectile && matches!(
                        ctx_data.as_ref().map(|(t, _, _)| t),
                        Some(game_core::combat::skill::ResolvedTargeting::Position { .. })
                    );
                    let is_caster_offset = !is_projectile && matches!(
                        ctx_data.as_ref().map(|(t, _, _)| t),
                    ...
                    if use_world_sensor {
                        ...
                        if is_ground_target {
                            ...
                            let point = match &targeting {
                                game_core::combat::skill::ResolvedTargeting::Position { point } => *point,
                                _ => unreachable!("is_ground_target already validated Position targeting"),
                            };
                            ...
                            if self.state.combat.hitboxes.arm_at_tick(execution_id, handle, self.current_tick) {
                                ...
                                self.emit_event(entity, EventPayload::HazardSpawned {
                                    execution_id: execution_id.0,
                                    ability_id,
                                    position: spawn_pos,
                                    radius,
                                });
                                audit!(self.state, Hitbox, AbilityTimeline, 3, Some(entity), "arm_ground");
                            } else {
                            ...
                            if self.state.combat.hitboxes.arm_at_tick(execution_id, handle, self.current_tick) {
                                ...
                                audit!(self.state, Hitbox, AbilityTimeline, 3, Some(entity), "arm_caster_offset");
                            } else {
                                self.physics.remove_sensor(handle);
                            }
                        } else {
                            ...
                            let projectile_speed = self.ability_prop(ability_id, |ad| ad.projectile_speed, None)
                                .unwrap_or(game_core::physics_constants::DEFAULT_PROJECTILE_SPEED);
                            let ability_max_range = self.ability_prop(ability_id, |ad| ad.max_range, None)
                                .unwrap_or(game_core::physics_constants::DEFAULT_ABILITY_MAX_RANGE);
                            ...
                            let max_range = match &targeting {
                                game_core::combat::skill::ResolvedTargeting::Position { point } => {
                                    ...
                                    (dx * dx + dz * dz).sqrt().min(ability_max_range)
                                }
                                _ => ability_max_range,
                            };
                            let handle = self.physics.spawn_world_sensor(
            ...
            AbilityAction::RemoveHitbox => {
                if let Some(removed) = self.state.combat.hitboxes.remove(execution_id) {
                    ...
                    if removed.projectile.is_some() || removed.world_sensor {
                        self.emit_event(entity, EventPayload::SkillObjectRemoved {
                            execution_id: execution_id.0,
                        });
                    }
                }
            ...
            AbilityAction::StanceBegin { dodge_active, rooted } => {
                ...
                if let Some(t) = self.tactical_mut(entity) {
                    ...
                    if *rooted {
                        t.movement_conditions.insert(game_core::combat::tactical::MovementConditions::ROOTED);
                    }
                }
                if *rooted {
            ...
            AbilityAction::StanceEnd => {
                ...
                if let Some(t) = self.tactical_mut(entity) {
                    ...
                    t.movement_conditions.remove(game_core::combat::tactical::MovementConditions::ROOTED);
            ...
            AbilityAction::RootForTicks { ticks } => {
                ...
                // If the entity is mid-air, inject a fall arc so gravity keeps acting.
                self.apply_gravity_if_airborne(entity);
                audit!(self.state, Tactical, AbilityTimeline, 3, Some(entity), "root_for_ticks");
            }
            AbilityAction::SetMovement { conditions } => {
                let needs_gravity_check = conditions.contains(
                ...
                audit!(self.state, Tactical, AbilityTimeline, 3, Some(entity), "set_movement");
            }
            ...
            AbilityAction::Telegraph { impact_delay } => {
                ...
                if let Some(ctx) = self.state.combat.executions.get(execution_id).cloned() {
                    ...
                    match &ctx.targeting {
                        game_core::combat::skill::ResolvedTargeting::Entity { target } => {
                            self.emit_event(*target, EventPayload::TelegraphWarning {
                                source: entity,
                                target: *target,
                                impact_tick,
                            });
                        ...
                        game_core::combat::skill::ResolvedTargeting::MultiLockOn { targets } => {
                            for &t in targets {
                                self.emit_event(t, EventPayload::TelegraphWarning {
                                    source: entity,
                                    target: t,
                                    impact_tick,
                                });
            ...
            AbilityAction::Cleanse { count } => {
                ...
                if let Some(idx) = self.state.entities.lookup(entity) {
                    ...
                    for ab in &removed {
                        self.emit_event(entity, EventPayload::BuffExpired { buff_id: ab.buff_id });
                        if let Some(cc) = ab.modifiers.cc_effect {
                            self.clear_cc_by_effect(entity, idx, cc);
                            self.emit_event(entity, EventPayload::CCCleared { cc_effect: cc, source: entity });
                        }
                    }
                    if !removed.is_empty() {
                        self.stats_dirty.insert(entity);
                        self.emit_event(entity, EventPayload::Cleansed { count: removed.len() as u32, source: entity });
                    }
                }
            }
            AbilityAction::ClearCC { cc_effect } => {
                ...
                if let Some(idx) = self.state.entities.lookup(entity) {
                    ...
                    self.emit_event(entity, EventPayload::CCCleared { cc_effect: *cc_effect, source: entity });
                }
            }
            AbilityAction::Stunbreak => {
                ...
                if let Some(idx) = self.state.entities.lookup(entity) {
                    ...
                    let all_cc = [CCEffect::Stun, CCEffect::Knockdown, CCEffect::Sleep, CCEffect::Silence, CCEffect::Fear];
                    for cc in &all_cc {
                        self.clear_cc_by_effect(entity, idx, *cc);
            ...
            AbilityAction::TeleportBehindTarget { distance } => {
                ...
                let data = self.state.combat.executions.get(execution_id)
                    .map(|ctx| (ctx.origin, ctx.targeting.clone()));
                let Some((caster_pos, targeting)) = data else { return; };
                ...
                let Some(target_t) = self.physics.get_transform(target) else { return; };
                ...
                let target_forward = Vec3f { x: yaw.sin(), y: 0.0, z: yaw.cos() };
                let raw_dest = Vec3f {
            ...
            AbilityAction::TeleportForward { distance } => {
                ...
                let Some((caster_pos, facing)) = data else { return; };
                let raw_dest = Vec3f {
