...
impl TickPipeline {
    ...
    pub(super) fn phase_controller_update(&mut self, intents: &[&PlayerIntent]) {
        ...
        for intent in intents {
            ...
            match &intent.action {
                ...
                IntentAction::TagTarget(target_id_raw) => self.handle_tag_target(entity_id, *target_id_raw),
                ...
                _ => {
                    ...
                    if self.is_silenced(idx) {
                        match &intent.action {
                            IntentAction::UseAbility(_) | IntentAction::ReleaseAbility(_) => continue,
                            _ => {}
                        }
                    }
                    match &intent.action {
                        IntentAction::UseAbility(_data) => self.handle_use_ability(intent, &mut cast_this_tick),
                        IntentAction::ReleaseAbility(ability_id) => self.handle_release_ability(entity_id, *ability_id, &mut cast_this_tick),
                        IntentAction::Block(data) => self.handle_block(entity_id, data),
                        IntentAction::Interact(target_id_raw) => self.handle_interact(entity_id, *target_id_raw),
    ...
    fn apply_movement(&mut self, entity_id: EntityId, dir: &MoveDir) {
        ...
        if let Some(idx) = self.state.entities.lookup(entity_id) {
            if self.state.combat.tactical[idx.as_usize()].arc_state.map_or(false, |a| a.gravity_only) {
                ...
                audit!(self.state, Tactical, Controller, 2, Some(entity_id), "air_dir");
                return;
            }
        }
                ...
                if !result.grounded {
                    ...
                    if t.arc_state.is_none() {
                        t.arc_state = Some(game_core::combat::tactical::ArcState {
                            velocity: Vec3f { x: vx, y: 0.0, z: vz },
            ...
            if let Some(result) = self.physics.move_character(entity_id, desired) {
                ...
                if result.grounded && vel_y <= 0.0 {
                    ...
                    if impact_speed >= game_core::physics_constants::FALL_DAMAGE_THRESHOLD {
                        let excess = impact_speed - game_core::physics_constants::FALL_DAMAGE_THRESHOLD;
                    ...
                    t.movement_conditions.remove(game_core::combat::tactical::MovementConditions::FLOATING);
                    ...
                    let attacker_id = self.state.combat.tactical[slot].arc_attacker.unwrap_or(entity_id);
    ...
    fn drive_fear_movement(&mut self) {
        ...
        let feared_entities: Vec<(EntityId, usize)> = self.state.combat.tactical.iter()
            .enumerate()
            .filter(|(_, t)| {
                t.movement_conditions.contains(MovementConditions::FEARED)
            })
            .filter_map(|(i, _)| {
                let id = self.state.entities.lookup_by_slot(i)?;
                Some((id, i))
        ...
        for (entity_id, slot) in feared_entities {
            ...
            if let Some(result) = self.physics.move_character(entity_id, desired) {
                self.state.combat.tactical[slot].is_grounded = result.grounded;
                audit!(self.state, Transform, Controller, 2, Some(entity_id), "fear_move");
            }
        }
    }
    ...
    fn apply_character_repulsion(&mut self) {
        ...
        let chars: Vec<CharInfo> = (0..self.state.entities.len())
            .filter_map(|i| {
                ...
                if !self.state.entities.is_active(idx) { return None; }
        ...
        for (ci, (px, pz)) in pushes.iter().enumerate() {
            if *px == 0.0 && *pz == 0.0 { continue; }
            let desired = Vec3f { x: *px, y: 0.0, z: *pz };
    ...
    fn drive_charging(&mut self) {
        ...
        let cc_cancelled: Vec<EntityId> = self.state.combat.charging.keys()
            .copied()
            .filter(|&eid| {
                self.state.entities.lookup(eid)
        ...
        for (&entity_id, charging) in self.state.combat.charging.iter_mut() {
            ...
            let charge_roots = self.abilities.get(charging.ability_id)
                .map(|ad| ad.charge_roots_while_charging)
                .unwrap_or(true);
            if let Some(idx) = self.state.entities.lookup(entity_id) {
                let t = &mut self.state.combat.tactical[idx.as_usize()];
                if charge_roots {
                    t.movement_conditions.insert(game_core::combat::tactical::MovementConditions::INPUT_LOCK);
                } else {
                    t.movement_conditions.remove(game_core::combat::tactical::MovementConditions::INPUT_LOCK);
                }
            }
        }
    ...
    fn handle_face_to(&mut self, entity_id: EntityId, dir: &game_schema::MoveDir) {
        self.set_entity_body_facing(entity_id, Vec3f {
            x: dir.dir_x,
            y: dir.dir_y,
            z: dir.dir_z,
        }, "face_to");
    ...
    fn handle_jump(&mut self, entity_id: EntityId) {
        let Some(idx) = self.state.entities.lookup(entity_id) else { return };
        let t_read = &self.state.combat.tactical[idx.as_usize()];
        if !t_read.is_grounded || t_read.arc_state.is_some() { return; }
        if self.is_rooted(idx) { return; }
        let t = &mut self.state.combat.tactical[idx.as_usize()];
        t.arc_state = Some(game_core::combat::tactical::ArcState {
            velocity: Vec3f { x: 0.0, y: game_core::physics_constants::JUMP_SPEED, z: 0.0 },
    ...
    fn handle_weapon_swap(&mut self, entity_id: EntityId) {
        ...
        let has_active_cast = self.state.combat.executions.active_ids()
            .iter()
            .any(|&eid| {
                self.state.combat.executions.get(eid)
                    .map_or(false, |ctx| ctx.caster == entity_id)
            });
        if has_active_cast { return; }
        // Reject if currently charging.
        if self.state.combat.charging.contains_key(&entity_id) { return; }
        // Reject if on weapon swap cooldown.
        if self.weapon_swap_cooldowns
            .get(&entity_id)
        ...
        let Some(loadout) = self.state.combat.loadouts.get_mut(idx) else { return };
        let new_set = loadout.swap();
        // Clear combo windows — weapon swap resets combos.
        self.state.combat.active_windows.retain(|&(eid, _), _| eid != entity_id);
        // Set swap cooldown.
        let cd = game_core::physics_constants::WEAPON_SWAP_COOLDOWN_TICKS;
        self.weapon_swap_cooldowns.insert(entity_id, TickId(self.current_tick.0 + cd as u64));
        self.emit_event(entity_id, EventPayload::WeaponSwapped { new_set });
        audit!(self.state, Tactical, Controller, 2, Some(entity_id), "weapon_swap");
    }

    fn handle_use_ability(&mut self, intent: &PlayerIntent, cast_this_tick: &mut HashSet<(EntityId, u32)>) {
        let entity_id = intent.entity_id;
        ...
        if cast_this_tick.contains(&cast_key) { return; }
        if self.state.combat.charging.contains_key(&entity_id) { return; }
        ...
        let targeting = match targeting_mode {
            TargetingMode::DirectionTarget => {
                let body_facing = self.physics.get_transform(entity_id)
                ...
                match &data.target {
                    game_schema::AbilityTarget::Direction(d) => {
                        let dir = Vec3f { x: d.x, y: d.y, z: d.z };
                        let Some(dir) = Self::normalize_direction(dir) else { return; };
                        ResolvedTargeting::Direction { dir }
                    }
                    game_schema::AbilityTarget::None => ResolvedTargeting::Direction { dir: body_facing },
                    _ => return,
                }
            }
            ...
            TargetingMode::GroundTarget => {
                ...
                let caster_pos = self.physics.get_transform(entity_id)
                    .map(|t| t.position)
            ...
            TargetingMode::RaycastStrict => {
                ...
                if let Some(hit) = self.physics.raycast(caster_pos, dir, max_range, Some(entity_id)) {
            ...
            TargetingMode::AimAssist => {
                ...
                match &data.target {
                    ...
                    game_schema::AbilityTarget::Direction(d) => {
                        ...
                        let inv_len = 1.0 / len_sq.sqrt();
                        let aim_dir = Vec3f { x: dir.x * inv_len, y: dir.y * inv_len, z: dir.z * inv_len };
                        ...
                        if let Some(hit) = self.physics.raycast(caster_pos, dir, max_range, Some(entity_id)) {
                            if matches!(hit.kind, ColliderKind::Hurtbox) {
                                ...
                                self.aim_assist_cone(entity_id, caster_pos, aim_dir, max_range, data.target_hint)
                            }
                        } else {
            ...
            TargetingMode::LockOn { max_targets } => {
                ...
                if self.is_on_cooldown(entity_id, ability_id) { return; }
                ...
                if let Some(session) = self.active_lock_on_sessions.remove(&entity_id) {
                    if session.ability_id == ability_id {
                        ...
                        for target in &session.tagged {
                            self.emit_event(entity_id, EventPayload::LockOnCanceled { source: entity_id, target: *target });
                        }
                        let timeout = self.ability_prop(ability_id, |ad| ad.lock_on_timeout_ticks, None)
                            .unwrap_or(LOCK_ON_SESSION_TIMEOUT_TICKS);
                        let timeout_at = TickId(self.current_tick.0 + timeout as u64);
                        self.active_lock_on_sessions.insert(entity_id, LockOnSession {
                            ability_id,
                            tagged: Vec::new(),
                            max_targets,
                            timeout_at,
                        });
                        self.emit_event(entity_id, EventPayload::LockOnSessionStarted { source: entity_id, ability_id });
                    }
                } else {
        ...
        if is_chargeable {
            ...
            let max_ticks = self.abilities.get(ability_id)
                ...
                .map(|t| t.min_ticks)
                .unwrap_or(1);
            let charge_roots = self.ability_prop(ability_id, |ad| ad.charge_roots_while_charging, true);
    ...
    fn aim_assist_cone(
        ...
        for (eid, transform) in self.physics.get_all_transforms() {
            ...
            if dist_sq > range_sq || dist_sq < 1e-6 { continue; }

            // Check if entity is within the cone.
            let inv_dist = 1.0 / dist_sq.sqrt();
            let to_target = Vec3f { x: dx * inv_dist, y: dy * inv_dist, z: dz * inv_dist };
    ...
    fn handle_release_ability(&mut self, entity_id: EntityId, ability_id: u32, cast_this_tick: &mut HashSet<(EntityId, u32)>) {
    ...
    fn handle_tag_target(&mut self, caster: EntityId, target_id_raw: u64) {
        ...
        if already_tagged || is_full { return; }
        // Target must exist in physics.
        let Some(target_pos) = self.physics.get_transform(target).map(|t| t.position) else { return };
        let Some(caster_pos) = self.physics.get_transform(caster).map(|t| t.position) else { return };
        // 3-D range check.
        let max_range = self.ability_prop(ability_id, |ad| ad.max_range, None)
            .unwrap_or(game_core::physics_constants::DEFAULT_ABILITY_MAX_RANGE);
        ...
        self.emit_event(caster, EventPayload::LockOnWarning { source: caster, target });
    }
