...
fn update_hazard_visuals(
    ...
    for (mut hv, mat_handle) in query.iter_mut() {
        ...
        let should_flash = flashes.iter().any(|(sid, aid)|
            *sid == hv.source_entity_id && *aid == hv.ability_id
        );
        if should_flash {
...
fn spawn_hitbox_visuals(
    ...
    use crate::ability_visuals::{visual_for, AbilityShape};
        ...
        if let Some(mat) = materials.get_mut(&mat_handle.0) {
            if vis.damage_flashed {
                ...
                let emissive_strength = (
                    HITBOX_PERIODIC_PULSE_EMISSIVE_BASE
                ...
                let (_, ability_color) = crate::ability_visuals::visual_for(vis.ability_id);
                let c = ability_color.to_srgba();
                mat.base_color = Color::srgba(c.red, c.green, c.blue, alpha_frac * HITBOX_BASE_ALPHA);
                mat.emissive = LinearRgba::new(
...
fn update_buff_flash_effects(
    mut commands: Commands,
    time: Res<Time>,
    mut query: Query<(bevy::ecs::entity::Entity, &mut BuffFlashEffect, &mut Transform, &MeshMaterial3d<StandardMaterial>)>,
    mut materials: ResMut<Assets<StandardMaterial>>,
) {
    let dt = time.delta_secs();
...
fn update_teleport_effects(
    mut commands: Commands,
    time: Res<Time>,
    mut query: Query<(bevy::ecs::entity::Entity, &mut TeleportFlashEffect, &mut Transform, &MeshMaterial3d<StandardMaterial>)>,
    mut materials: ResMut<Assets<StandardMaterial>>,
) {
    let dt = time.delta_secs();
    for (entity, mut effect, mut tf, mat_handle) in query.iter_mut() {
