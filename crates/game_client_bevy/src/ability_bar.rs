...
fn skill_shape_to_client(shape: game_core::combat::skill::SkillShape) -> AbilityShape {
    ...
    match shape {
        SkillShape::CapsuleSweep => AbilityShape::Capsule {
            radius: 0.75,
            half_height: 1.0,
        },
        SkillShape::LineSweep => AbilityShape::Capsule {
            radius: 0.5,
            half_height: 3.0,
        },
        SkillShape::Cone => AbilityShape::Capsule {
            radius: 0.75,
            half_height: 1.0,
        },
    ...
    commands
        .spawn((
            Node {
                ...
                margin: UiRect {
                    left: Val::Px(-((9.0 * 60.0 + 8.0 * 4.0) / 2.0)),
                    ..default()
                },
...
impl SkillMenuState {
    ...
    fn filtered_ability_ids(&self) -> Vec<u32> {
        all_abilities()
            .iter()
            .filter(|def| {
                self.active_category == SkillMenuCategory::All
                    || def.category == self.active_category
            })
            .map(|def| def.id)
            .collect()
    }
...
fn handle_menu_clicks(
    ...
    mut interaction_q: Query<
        (&Interaction, &SkillMenuButton, &mut BackgroundColor),
        Changed<Interaction>,
    >,
    mut menu_state: ResMut<SkillMenuState>,
) {
...
fn handle_filter_clicks(
    mut interaction_q: Query<
        (&Interaction, &SkillMenuFilterButton, &mut BackgroundColor),
        Changed<Interaction>,
    >,
    mut menu_state: ResMut<SkillMenuState>,
) {
...
fn handle_scroll_controls(
    ...
    mut interaction_q: Query<
        (&Interaction, &SkillMenuScrollButton, &mut BackgroundColor),
        Changed<Interaction>,
    >,
    mut menu_state: ResMut<SkillMenuState>,
) {
...
fn update_skill_menu_visibility(
    ...
    let Ok(mut node) = root_q.get_single_mut() else {
        return;
    };
    if menu_state.active_slot.is_some() {
    ...
    let start = menu_state.scroll_offset.min(
        filtered_ids
            .len()
            .saturating_sub(SkillMenuState::VISIBLE_ROWS),
    );
    ...
    for (entry, mut entry_node, mut bg) in entry_q.iter_mut() {
        ...
        entry_node.display = if visible {
            Display::Flex
        } else {
            Display::None
        };

        if let Some(def) = all_abilities()
            .iter()
            .find(|def| def.id == entry.ability_id)
        {
            bg.0 = if visible && menu_state.active_category != SkillMenuCategory::All {
                match def.color {
                    Color::Srgba(tint) => {
                        Color::srgba(tint.red * 0.25, tint.green * 0.25, tint.blue * 0.25, 0.95)
                    }
    ...
    let Ok(mut scroll_text) = scroll_label_q.get_single_mut() else {
        return;
    };
