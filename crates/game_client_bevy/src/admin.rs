...
/// F10 = Spawn an NPC 5 units in front of the player (calls spawn_npc reducer).
fn admin_spawn_npc(
    ...
    // Position: 5 units in front of the player (negative Z in default)
    ...
    let max_hp = 100.0_f32;
    match stdb.conn.reducers.spawn_npc(px, py, pz, max_hp) {
        Ok(()) => {
            status.message = format!("Spawned NPC at ({px:.1}, {py:.1}, {pz:.1}) HP={max_hp}");
            log::info!("{}", status.message);
        }
        Err(e) => {
            status.message = format!("spawn_npc failed: {e}");
