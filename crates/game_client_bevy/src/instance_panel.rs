...
impl Plugin for InstancePanelPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(Update, (toggle_instance_panel, update_instance_panel, handle_instance_keys));
    }
}
...
fn update_instance_panel(
    ...
    let instances: Vec<_> = stdb.conn.db.instance().iter()
    ...
    if instances.is_empty() {
        ...
        for inst in &instances {
            let member_count = stdb.conn.db.instance_membership().iter()
...
fn handle_instance_keys(
    ...
    if keyboard.just_pressed(KeyCode::F6) {
        // TODO: After next schema deploy + binding regen, switch to:
        //   stdb.conn.reducers.debug_create_instance(entity_id)
        // which auto-creates + auto-joins.  For now we call create_instance
        // (works because the game client IS a registered player) and the
        // user must join manually via the panel or F5 list.
        match stdb.conn.reducers.create_instance("test_dungeon_01".into(), 4) {
