//! Multi-client integration tests for a running TickForge server.
//!
...
pub fn run_tests(config: ClientConfig) -> i32 {
    ...
    info!("═══ TickForge Multi-Client Integration Tests ═══");
...
fn run_coexistence_tests(config: &ClientConfig, results: &Arc<Mutex<TestResults>>) {
    ...
    let both_ready = wait_for_both(&client_a.conn, &client_b.conn, 15000, || {
    ...
    let a_sees_b = client_a
        .conn
        .db()
        .nearby_transforms()
        .iter()
        .any(|t| t.entity_id == eid_b);
    let b_sees_a = client_b
        .conn
        .db()
        .nearby_transforms()
        .iter()
    ...
    let _ = client_a
        .conn
        .reducers()
        .submit_intent(eid_a, seq + 1, IntentAction::Stop, 0);
    ...
    match b_view_of_a {
        Some(t) if t.pos_x > 0.01 => {
            r.pass(&format!(
                "M3  client B observed A's movement (pos_x={:.2})",
                t.pos_x
            ));
        }
        Some(t) => {
            r.warn_msg(&format!(
                "M3  client B sees A but pos_x={:.3} (may need more ticks)",
                t.pos_x
            ));
        }
        None => {
...
fn call_reducer_wait(
    ...
    call(Arc::clone(&done), Arc::clone(&result)).map_err(|e| format!("send failed: {e}"))?;
...
fn run_party_lifecycle_tests(config: &ClientConfig, results: &Arc<Mutex<TestResults>>) {
    ...
    let create_result = call_reducer_wait(&client_a.conn, 5000, |done, result| {
        client_a
            .conn
            .reducers()
            .create_party_then(move |_ctx, res| {
    ...
    let invite_result = call_reducer_wait(&client_a.conn, 5000, |done, result| {
        client_a
            .conn
            .reducers()
            .invite_to_party_then(eid_b, move |_ctx, res| {
                *result.lock().unwrap() = match res {
                    Ok(Ok(())) => Ok(()),
                    Ok(Err(e)) => Err(e),
                    Err(e) => Err(format!("{e}")),
    ...
    let accept_result = call_reducer_wait(&client_b.conn, 5000, |done, result| {
        client_b
            .conn
            .reducers()
            .accept_party_invite_then(invite_id, move |_ctx, res| {
                *result.lock().unwrap() = match res {
                    Ok(Ok(())) => Ok(()),
                    Ok(Err(e)) => Err(e),
                    Err(e) => Err(format!("{e}")),
    ...
    if member_count == 2 {
        ...
        r.warn_msg(&format!(
            "P2d  expected 2 party members, found {member_count}"
        ));
    }
    drop(r);
    ...
    let disband_result = call_reducer_wait(&client_a.conn, 5000, |done, result| {
        client_a
            .conn
            .reducers()
            .disband_party_then(move |_ctx, res| {
                *result.lock().unwrap() = match res {
                    Ok(Ok(())) => Ok(()),
                    Ok(Err(e)) => Err(e),
                    Err(e) => Err(format!("{e}")),
